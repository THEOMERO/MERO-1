...
local url , res = https.request('https://hso.mohammed-api.com/ApI_BoAykX.php/?id='..User..'')
data = JSON.decode(url)
if data.Ch_Member. MEROTeam ~= true then
Var = false
else
Var = true
end
...
function plugin_ MEROa(msg)
for v in io.popen('ls File_Bot'):lines() do
if v:match(".lua$") then
plugin = dofile("File_Bot/"..v)
if plugin. MEROa and msg then
pre_msg = plugin. MEROa(msg)
end
end
end
...
--------------------------------------------------------------------------------------------------------------
function Source MERO(msg,data) -- بداية العمل
if msg then
...
send(msg.chat_id_, msg.id_,'-  ، لايمكنك استخدام البوت 💕.\n- ﭑشترك اولاً 💕 • [['..database:get(bot_id..'add:ch:username')..']] .')
return false
end
if SudoBot(msg) then
...
send(msg.chat_id_, msg.id_, Source MEROr) 
end
end
...
os.execute('wget https://raw.githubusercontent.com/Team MERO/ MERO/master/ MERO.lua')
...
send(msg.chat_id_,0, "┆عذرأ يا  ◃ ["..data.first_name_.."](T.ME/MERO170)    ܰ\n┆الملصق التي ارسلتهاا تم منعهاا من المجموعةه    ܰ\n" ) 
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
local UserName = (b.username_ or "MERO170")
...
-------------------------------------LOKRAND - @bbbbl - @Y07BOT - @VVWVV3 - ['..database:get(bot_id..'add:ch:username')..'] ---------------------------------------
...
sendDocument(msg.chat_id_, msg.id_,0, 1, nil, './ MERO.lua', '┆ ماوصيك اخمط 😹😭  ܰ ')
end
...
܀⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤܀
𖤂┆[CHanneL SourcE](t.me/MERO170) 𖥠 .
]]
...
--------------------------------------------------------------------------------------------------------------
Source MERO(data.message_,data)
plugin_ MEROa(data.message_)
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
